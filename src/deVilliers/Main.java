                ...
                for (int k = 0; k <= mutationrates.size() - 1; k++) {
                    for (int m = 0; m <= crossoverrates.size() - 1; m++) {
                        for (int n = 0; n <= selectionOperators.size() - 1; n++) {
                            Population model1 = new Population(sizes.get(i), "Rsquared > 0.98 or Generations == 1000", mutationrates.get(k), mutationmagnitudes.get(j), crossoverrates.get(m), selectionOperators.get(n));
                            int igen = 1;
                            while ((model1.population.get(0).rsquared < 0.98) && (igen < 1000)) {
